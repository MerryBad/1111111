...
class ResnetPeriodEstimator(tf.keras.models.Model):
  ...
  def call(self, x):
    ...
    print('1:',x) # Tensor("Reshape_1:0", shape=(1, 64, 7, 7, 1024), dtype=float32)
    ...
    print('2:',x) #Tensor("Relu:0", shape=(1, 64, 7, 7, 512), dtype=float32)
    x = tf.reduce_max(x, [2, 3])
    print('3:',x) #Tensor("Max:0", shape=(1, 64, 512), dtype=float32)
    # Reshape and prepare embs for output.
    final_embs = x

    # Get self-similarity matrix.
    x = get_sims(x, self.temperature)
    print('4:',x) # Tensor("ExpandDims:0", shape=(1, 64, 64, 1), dtype=float32)

    # 3x3 conv layer on self-similarity matrix.
    x = self.conv_3x3_layer(x)
    x = tf.reshape(x, [batch_size, self.num_frames, -1])
    within_period_x = x
    print('5:',x) # Tensor("Reshape_3:0", shape=(1, 64, 2048), dtype=float32)


    # Period prediction.
    x = self.input_projection(x)
    print('6:', x)#6: Tensor("dense_40/BiasAdd:0", shape=(1, 64, 512), dtype=float32)
    x += self.pos_encoding
    print('7:', x)#7: Tensor("add:0", shape=(1, 64, 512), dtype=float32)
    for transformer_layer in self.transformer_layers:
      x = transformer_layer(x)
    print('8:',x)#8: Tensor("transformer_layer_4/add_1:0", shape=(1, 64, 512), dtype=float32)
    x = flatten_sequential_feats(x, batch_size, self.num_frames)
    print('9:',x)#9: Tensor("Reshape_4:0", shape=(1, 64, 512), dtype=float32)
    for fc_layer in self.fc_layers:
      x = self.dropout_layer(x)
      x = fc_layer(x)

    print('10:',x)# 10: Tensor("dense_56/BiasAdd:0", shape=(1, 64, 32), dtype=float32)

    # Within period prediction.
    within_period_x = self.input_projection2(within_period_x)
    print('with_1:', within_period_x)#with_1: Tensor("dense_61/BiasAdd:0", shape=(1, 64, 512), dtype=float32)
    ...
    for transformer_layer in self.transformer_layers2:
      ...
      print('with_2:', within_period_x)#with_2: Tensor("transformer_layer_7/add_1:0", shape=(1, 64, 512), dtype=float32)
    within_period_x = flatten_sequential_feats(within_period_x,
                                               batch_size,
                                               self.num_frames)
    print('with_3:', within_period_x)#with_3: Tensor("Reshape_5:0", shape=(1, 64, 512), dtype=float32)
    ...
    print('with_4:', within_period_x)#with_4: Tensor("dense_79/BiasAdd:0", shape=(1, 64, 1), dtype=float32)
...
def get_sims(embs, temperature):
  ...
  print('embs=',embs)    # embs = Tensor("Reshape_2:0", shape=(1, 64, 512), dtype=float32)
  def _get_sims(embs):
    """Calculates self-similarity between sequence of embeddings."""
    dist = pairwise_l2_distance(embs, embs)
    print('dist=', dist)    # dist = Tensor("map/while/Maximum:0", shape=(64, 64), dtype=float32)
    sims = -1.0 * dist
    return sims
...
def pairwise_l2_distance(a, b):
  ...
  print('a:',a) #a: Tensor("map/while/TensorArrayV2Read/TensorListGetItem:0", shape=(64, 512), dtype=float32)
  print('b:',b) #b: Tensor("map/while/TensorArrayV2Read/TensorListGetItem:0", shape=(64, 512), dtype=float32)

  norm_a = tf.reduce_sum(tf.square(a), 1)
  print('norma:',norm_a)#norma: Tensor("map/while/Sum:0", shape=(64,), dtype=float32)
  norm_a = tf.reshape(norm_a, [-1, 1])
  print('norma:',norm_a)#norma: Tensor("map/while/Reshape:0", shape=(64, 1), dtype=float32)
